...
/*
  ...
  - Настройка времени:
    ...
    - Клик по "выбору" - переход к редактированию: часов (время) => минут (время) => часов (будильник) => минут (будильник)
    - Клик "больше" и "меньше" - изменение времени
    - Удержать "выбор" - возврат к режиму часов. Переведено будет то время (текущее или будильник), которое было в режиме редактирования перед выходом

  - Управление эффектами В РЕЖИМЕ ЧАСОВ:
    - Клик по левой кнопке включает и выключает будильник
      - Одиночный сигнал - будильник отключен
      - Двойной сигнал и отображение времени будильника - включен
    - Удержание центральной кнопки включает и выключает "глюки"
      - Одиночный краткий сигнал и индикация 0000 - выключены
      - Два очень кратких сигнала и индикация 1111 - включены
...
/*
  ...
  Версия 2.7:
  - Добавлена возможность подключения обычного динамика
  - Исправлены некоторые баги с задержками после демонстрации режима и времени будильника
  - При настройке времени будильника настройка времени не производится
  - Демонстрируется 0 или 1 при включении\отключении режима глюков

  Версия 2.8:
  - Оптимизация работы с аппаратными таймерами
  - Добавлена возможность проигрывания мелодии будильником
  - Обновлена справка в начале файла
*/
...
#define DOT_BRIGHT_N 16     // яркость точки ночная (1 - 255)
